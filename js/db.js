...
const localDB = new PouchDB('innercalm');
...
async function migrateFromLocalStorage() {
  const raw = localStorage.getItem('innercalm_state');
  if (!raw) return false;
  ...
  try {
    ...
    localStorage.removeItem('innercalm_state');
...
function getSyncUrl() {
  return (localStorage.getItem('innercalm_sync_url') || '').replace(/\s+/g, '');
}

function setSyncUrl(url) {
  if (url) localStorage.setItem('innercalm_sync_url', url);
  else localStorage.removeItem('innercalm_sync_url');
}
